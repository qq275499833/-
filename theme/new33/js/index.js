...
function loadData(data) {
    var app = new Vue({
        ...
        data: {
            ...
            item:data.detail.manyoff?JSON.parse(data.detail.manyoff)[0].salecount:"",
        ...
        methods: {
            //商品属性、动态添加class
            No1:function (index,giftcount,price) {
        ...
        //    去下单
            one_submit: function () {
                ...
                if(this.detail.giftGoodsIds&&this.giftLength) {
                    for (var i in this.giftSelect) {
                        for (var j in this.giftSelect[i]) {
                            var gift = this.giftSelect[i][j];
                            if (!gift.id || !gift.sku || !prod.img) {
                                tools.show_msg(selectGiftStr + (parseInt(j) + 1));
                                return;
                            }
                        }
                    }
