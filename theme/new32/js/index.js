...
function loadData(data) {
    var app = new Vue({
        ...
        data: {
            ...
            //本地存储的名称
            show_cart:false,
            show_img:data.detail.head_video?data.detail.head_video:data.detail.head_imgs[0],
            localCart_data: data.detail.sitedir + 'mb',
        ...
        created: function () {
            //页面加载取数据
            if (JSON.parse(localStorage.getItem(this.localCart_data)) !== null) {
                this.cartarr= JSON.parse(localStorage.getItem(this.localCart_data));
            }
        ...
        methods: {
            tab_img(index){
                if(this.detail.head_video){
                    var src=this.detail.head_imgs[index];
                    this.show_img='<img  src="' +this.cdnbase  + src+ '">'
                }else{
                    this.show_img=this.detail.head_imgs[index];
                }
            },
            //折扣方法，保留一位小数
            ...
            calc_price_nocart: function () {
                ...
                for (var i in this.cartarr) {
                    var prod = this.cartarr[i];
                    total_number+=prod.number;
                    this.realPrice += prod.price*prod.number;
                    //多件优惠
                ...
                this.realPrice = this.realPrice - this.manyoff_price;
                //一口价
                for (var i in this.manyoff) {
                    if (this.manyoff[i].salecount === total_number) {
                        ...
                        success: function (data) {
                            ...
                            if (data.Error) {
                                ...
                                app.$nextTick(function () {
                                    ...
                                    show_successwin();
                                    //删除数据
                                    localStorage.removeItem(this.localCart_data);
                                })
                            }
                        }
                    });
            ...
            add_cart() {
                ...
                tools.show_msg(add_success);
                $('body').css('overflow','hidden');
                var that = this;
                $('.cart').css('display','block');
                $('.home_zixun_fixed').css('display','none');
                for(var i in this.cartarr){
                    if(this.cartarr[i].specname===this.prods[0].specname&&this.cartarr[i].option1===this.prods[0].option1&&this.cartarr[i].option2===this.prods[0].option2){
                        if(this.cartarr[i].gift[0].option1===giftArr[0].option1&&this.cartarr[i].gift[0].option2===giftArr[0].option2&&this.cartarr[i].gift[0].specname===giftArr[0].specname){
                           this.cartarr[i].number+=1;
                            this.calc_price_nocart();
                            //存数据
                            localStorage.setItem(this.localCart_data, JSON.stringify(this.cartarr));
                            return;
                        }
                    }
                }
        ...
        //    我来评价
            evaluation(){
                this.evaluation_flag=!this.evaluation_flag;
                if(this.evaluation_flag){
                    $('.evalua').text(cancel);
                }else{
                    $('.evalua').text(write);
                }
            ...
            showCart(){
                ...
                if(this.show_cart===true){
                    $('.up_down').css('background','url("/theme/new32/images/up.png")no-repeat');
                    $('.up_down').css('background-size','20px');
                }else{
                    $('.up_down').css('background','url("/theme/new32/images/down.png")no-repeat');
                    $('.up_down').css('background-size','20px');
                }
            }
