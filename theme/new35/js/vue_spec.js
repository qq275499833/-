...
function loadData(data) {
    var app = new Vue({
        ...
        data: {
            ...
            qualificationList: ['/theme/new35/images/1.png', '/theme/new35/images/2.png', '/theme/new35/images/3.png', '/theme/new35/images/4.png', '/theme/new35/images/5.png']
        },
        created: function () {
            this.buy_info();
            // 评价滚动
            setInterval(this.evaluation_scroll, 100);
            //选择城市
        ...
        mounted: function () {
            if($('.head_nav1').height()<1000){
                $('#home_slider').css('marginTop',$('.head_nav1').height()+'px');
            }
            //swiper
        ...
        methods: {
            //购买信息
            buy_info: function () {
                $.get(this.apiserver + "/getpurchaseinfo/" + this.detail.language).success(function (data) {
                    if (data.Error) {
            ...
            evaluation_scroll() {
                ...
                if (pjcon2 === undefined) {
                    ...
                    if (this.pj2 <= -this.pj1) {
                        this.pj2 = 0;
                        this.buyInfoList.push(this.buyInfoList[0]);
                        this.buyInfoList.shift();
            ...
            submit_order_nocart: function () {
                if (!this.flag1) {
                    ...
                    submiting = true;
                    tools.show_loading(submitingStr, 100);
                    $.ajax({
                        type: "POST",
                        url: '/createorder',
                        ...
                        success: function (data) {
                            ...
                            if (data.Error) {
                                ...
                                app.$nextTick(function () {
                                    ...
                                    this.orderprice = data.orderInfo.orderprice;show_successwin();
            ...
            go_top() {
                document.getElementById("home_slider").scrollIntoView(true);
            },
            //手机端客服
            show(){
                this.phone_kf=!this.phone_kf;
            },
            go_back(){
                $('.submit').css('display','none');
                $('.home_page1').css('display','block');
                $('.service').css('display','block');
                $('.footer_fixed').css('display','block');
            }
        }
    });
}
