...
function loadData(data) {
    var app = new Vue({
        ...
        mounted: function () {
            ...
            //判断manyoff起卖件数
            this.No1(this.item,this.giftCount);
        },
        methods: {
            //返回主页面
            bk:function(){
            ...
            discount: function () {
               ...
               let v1=this.value.toString().split('.');
            ...
            submit_order_nocart: function () {
                ...
                if (this.detail.money !== '￥' && this.detail.money !== 'S$') {
                    ...
                    if (this.clientinfo.area === '区县' || this.clientinfo.area === '區縣' || this.clientinfo.area === 'City' || this.clientinfo.area === 'เมือง' || this.clientinfo.area === '區' || this.clientinfo.area === undefined ||this.clientinfo.area === ""||this.clientinfo.city ==='都道府県を選択') {
                        $('#area').focus();
                ...
                $.get("/querycomments/" + this.detail.id).success(function (data) {
                    if (data.Error) {
                        ...
                        app.$nextTick(function () {
                            ...
                            this.com1=data.comments.splice(0,5);
                            this.com2=data.comments.splice(0,5);
                            this.com3=data.comments.splice(0,5);
                        })
                    }
                })
            },
            ...
            xs1:function () {
                this.mn1=!this.mn1;
                if(this.mn1==true){
                    $('.ic').css('background','url("/theme/new25/images/up.png")no-repeat');
                    $('.ic').css('background-size','27px');
                }else{
                    $('.ic').css('background','url("/theme/new25/images/down.png")no-repeat');
                    $('.ic').css('background-size','27px');
                }
